...
public class SudokuBase {
  ...
  public static void initGrilleComplete(int[][] gComplete) {      ///EXTENSION 3.4
    int[][] grilleRemplie = {
            { 6, 2, 9, 7, 8, 1, 3, 4, 5 },
            { 4, 7, 3, 9, 6, 5, 8, 1, 2 },
            { 8, 1, 5, 2, 4, 3, 6, 9, 7 },
  ...
  public static int initPartie(int[][] gSecret, int[][] gHumain, int[][] gOrdi, boolean[][][] valPossibles,
    ...
    System.out.println("Voulez vous remplir la grille de l'ordinateur : OUI via le chat (1) / OUI via fichier (2) / NON (3)");
  ...
  public static int tourHumain(int[][] gSecret, int[][] gHumain) {
    ...
    System.out.println(
            "\n" + "Vous êtes sur un trou, que souhaitez vous faire : Une proposition (1) / Un joker (2) ?");
    int choix = saisirEntierMinMax(1, 2);

    if (choix == 1) { // SI choix = Proposition (1)
  ...
  public static void partie() {
    ...
    while (nbtrou != 0) { // PARTIE
      // HUMAIN
      System.out.println("\n" + "Votre Grille :");
      penaliteHumain += tourHumain(grilleCompleteChoisieParOrdi,
              grilleIncompleteChoisieParOrdi);
      System.out.println("Pénalité(s) Joueur :" + penaliteHumain);

      // ORDI
      penaliteOrdi += tourOrdinateur(grilleIncompleteChoisieParHumain, valPossibles, nbValPoss);
      System.out.println("\n" + "Pénalité(s) Ordi:" + penaliteOrdi);
      System.out.println("L'ordi a joué, voici sa grille :");
